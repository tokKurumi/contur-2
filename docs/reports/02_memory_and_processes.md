...
Ключевая задача этого блока — соединить:

- безопасный доступ к памяти через интерфейсы и `Result<T>`;
...
Объектная модель процесса в проекте строится из двух связанных сущностей — `PCB` и
`ProcessImage`.
...
Минимальный сценарий проверки memory/process-стека:
...
Память и модель процесса образуют рабочую системную модель: память изолирована и управляется
через MMU и таблицы страниц, а процесс несёт формализованное состояние, тайминги и контекст
исполнения. Это даёт стабильный слой, поверх которого корректно работают планирование,
диспетчеризация и исполнение программ без нарушения инвариантов памяти и lifecycle-переходов.
