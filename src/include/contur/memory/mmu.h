...
namespace contur {
    ...
    public:
        ...
        // IMMU interface
        [[nodiscard]] Result<Block> read(ProcessId processId,
                                         MemoryAddress virtualAddress) const override;
        [[nodiscard]] Result<void> write(ProcessId processId, MemoryAddress virtualAddress,
                                         const Block& block) override;
        [[nodiscard]] Result<MemoryAddress> allocate(ProcessId processId,
                                                     std::size_t pageCount) override;
        [[nodiscard]] Result<void> deallocate(ProcessId processId) override;
        [[nodiscard]] Result<void> swapIn(ProcessId processId,
                                          MemoryAddress virtualAddress) override;
        [[nodiscard]] Result<void> swapOut(ProcessId processId,
                                           MemoryAddress virtualAddress) override;
