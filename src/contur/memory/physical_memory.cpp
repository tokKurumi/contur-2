...
namespace contur {
    ...
    struct PhysicalMemory::Impl {
        ...
        explicit Impl(std::size_t cellCount) : cells(cellCount) {}
    };
    ...
    Result<void> PhysicalMemory::writeRange(MemoryAddress startAddress,
    ...
    Result<std::vector<Block>> PhysicalMemory::readRange(MemoryAddress startAddress,
                                                         std::size_t count) const
    {
        ...
        std::vector<Block> result(impl_->cells.begin() + startAddress,
                                  impl_->cells.begin() + startAddress + count);
